...
from utils.db import create_db_engine, create_ssh_tunnel, init_db
from utils.helper import build_timeseries, points_in_boundaries


def main(city: Literal["rotterdam", "amsterdam"], insert: bool):
                ...
                while True:
                    try:
                        if (data := fetch_data()) is not None:
                            n_rows = len(data)
                            data.pipe(points_in_boundaries, BOUNDARIES).pipe(
                                build_timeseries
                            ).to_sql(
                                TABLE_NAME,
                                conn,
                            ...
                            logging.info(
                                f"Pushed {n_rows} new rows | Waiting for new data"
                            )

                        else:
                            logging.warning(
                                "No new data has been pulled. Waiting for new data"
                            )
...
if __name__ == "__main__":
    logging = logger.setup_logger("etl_pipeline")
    import argparse
